...
class Campaign extends ActiveRecord
{
    ...
    {
        // 1.3.8.0 - stop sending to subs added while campaign running
        // but only for non-merged lists since these can take a while to merge
        // also people can schedule campaigns in the past, which can also cause issues
        /*
        $isMerged  = !empty($this->list) && !empty($this->list->merged) && $this->list->merged == Lists::TEXT_YES;
        $inThePast = strtotime($this->send_at) < strtotime('-1 hour');
        if (!$isMerged && !$inThePast) {
            $criteria->addCondition('t.date_added <= :campaign_send_at_date');
            $criteria->params[':campaign_send_at_date'] = $this->send_at;
        }
        */
