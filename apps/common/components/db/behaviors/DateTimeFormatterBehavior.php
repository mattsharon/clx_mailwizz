...
class DateTimeFormatterBehavior extends CActiveRecordBehavior
{
    ...
    public function getTimeZone()
    {
        ...
        $app = Yii::app();
        if ($app->hasComponent('user') && $app->user->getId() > 0 && !$this->owner->isNewRecord) {
            $this->_timeZone = $app->user->getModel()->timezone;
        }
        
        $apps = $app->apps;
        if ($apps->isAppName('customer') && $app->hasComponent('customer') && $app->customer->getId() > 0 && !$this->owner->isNewRecord) {
            $this->_timeZone = $app->customer->getModel()->timezone;
        }
