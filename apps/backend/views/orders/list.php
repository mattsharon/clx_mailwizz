...
if ($viewCollection->renderContent) { ?>
    <div class="box box-primary borderless">
        <div class="box-header">
            <div class="pull-left">
                <h3 class="box-title">
                    <?php echo IconHelper::make('glyphicon-credit-card') .  $pageHeading;?>
                </h3>
            </div>
            <div class="pull-right">
                <?php echo HtmlHelper::accessLink(IconHelper::make('create') . Yii::t('app', 'Create new'), array('orders/create'), array('class' => 'btn btn-primary btn-flat', 'title' => Yii::t('app', 'Create new')));?>
                <?php echo HtmlHelper::accessLink(IconHelper::make('refresh') . Yii::t('app', 'Refresh'), array('orders/index'), array('class' => 'btn btn-primary btn-flat', 'title' => Yii::t('app', 'Refresh')));?>
            </div>
            ...
            if ($collection->renderGrid) {
                $this->widget('zii.widgets.grid.CGridView', $hooks->applyFilters('grid_view_properties', array(
                    ...
                    'columns' => $hooks->applyFilters('grid_view_columns', array(
                        array(
                            'name'  => 'order_uid',
                            'value' => 'HtmlHelper::accessLink($data->uid, array("orders/update", "id" => $data->order_id))',
                            'type'  => 'raw',
                        ),
                        array(
                            'name'  => 'customer_id',
                        ...
                        array(
                            'name'  => 'plan_id',
                            'value' => 'HtmlHelper::accessLink($data->plan->name, array("price_plans/update", "id" => $data->plan_id))',
                            'type'  => 'raw',
                        ),
                        array(
                            'name'  => 'promo_code_id',
                            'value' => '!empty($data->promo_code_id) ? HtmlHelper::accessLink($data->promoCode->code, array("promo_codes/update", "id" => $data->promo_code_id)) : "-"',
                            'type'  => 'raw',
                        ),
                        array(
                            'name'  => 'subtotal',
